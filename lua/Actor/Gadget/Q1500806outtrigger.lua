local L0_1, L1_1, L2_1, L3_1, L4_1, L5_1, L6_1, L7_1, L8_1
L0_1 = require
L1_1 = "Actor/ActorCommon"
L0_1(L1_1)
L0_1 = require
L1_1 = "Actor/Gadget/GadgetActorProxy"
L0_1 = L0_1(L1_1)
L1_1 = class
L2_1 = "Q1500806outtrigger"
L3_1 = L0_1
L1_1 = L1_1(L2_1, L3_1)
L1_1.defaultAlias = "Q1500806outtrigger"
L2_1 = require
L3_1 = "Quest/Client/Q15008ClientConfig"
L2_1 = L2_1(L3_1)
L3_1 = nil
L4_1 = L2_1.SubIDs
L5_1 = L2_1.Datas
L6_1 = L2_1.Npcs
L7_1 = L2_1.Points
function L8_1(A0_2)
  local L1_2, L2_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - OnPostDataPrepare"
  L1_2(L2_2)
  L3_1 = A0_2
end
L1_1.OnPostDataPrepare = L8_1
function L8_1(A0_2)
  local L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - OnPostComponentPrepare"
  L1_2(L2_2)
  L1_2 = L3_1
  L2_2 = L1_2
  L1_2 = L1_2.AddPolygonTrigger
  L3_2 = 0
  L4_2 = "Natlan_Quest_fail_1500806"
  L5_2 = A0_2.TriggerIn
  L6_2 = A0_2.TriggerOut
  L7_2 = A0_2.PreTriggerIn
  L8_2 = A0_2.LightUpTriggerIn
  L9_2 = 15008
  L1_2(L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2)
end
L1_1.OnPostComponentPrepare = L8_1
function L8_1(A0_2)
  local L1_2, L2_2, L3_2, L4_2, L5_2, L6_2, L7_2, L8_2, L9_2, L10_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - TriggerIn"
  L1_2(L2_2)
  L1_2 = actorMgr
  L2_2 = L1_2
  L1_2 = L1_2.GetActor
  L3_2 = "15008"
  L1_2 = L1_2(L2_2, L3_2)
  L3_2 = L1_2
  L2_2 = L1_2.GetQuestVar
  L4_2 = 1500806
  L5_2 = 4
  L2_2 = L2_2(L3_2, L4_2, L5_2)
  if L2_2 == 0 then
    L3_2 = actorMgr
    L4_2 = L3_2
    L3_2 = L3_2.GetActor
    L5_2 = "15008"
    L3_2 = L3_2(L4_2, L5_2)
    L5_2 = L3_2
    L4_2 = L3_2.EnablePlayerInputByQuest
    L6_2 = false
    L4_2(L5_2, L6_2)
    L4_2 = actorMgr
    L5_2 = L4_2
    L4_2 = L4_2.GetActor
    L6_2 = "15008"
    L4_2 = L4_2(L5_2, L6_2)
    L6_2 = L4_2
    L5_2 = L4_2.QuestNarratorTask
    L7_2 = L5_1.NarratorWithId_1500804
    function L8_2(A0_3)
      local L1_3, L2_3, L3_3, L4_3, L5_3, L6_3, L7_3, L8_3
      L1_3 = actorMgr
      L2_3 = L1_3
      L1_3 = L1_3.GetActor
      L3_3 = "15008"
      L1_3 = L1_3(L2_3, L3_3)
      L3_3 = L1_3
      L2_3 = L1_3.TransmitPlayerByQuestId
      L4_3 = 1500806
      L5_3 = 7
      function L6_3(A0_4)
        local L1_4, L2_4, L3_4, L4_4
        L1_4 = actorMgr
        L2_4 = L1_4
        L1_4 = L1_4.GetActor
        L3_4 = "15008"
        L1_4 = L1_4(L2_4, L3_4)
        L3_4 = L1_4
        L2_4 = L1_4.EnablePlayerInputByQuest
        L4_4 = true
        L2_4(L3_4, L4_4)
      end
      L7_3 = nil
      L8_3 = nil
      L2_3(L3_3, L4_3, L5_3, L6_3, L7_3, L8_3)
    end
    L5_2(L6_2, L7_2, L8_2)
    L5_2 = actorMgr
    L6_2 = L5_2
    L5_2 = L5_2.GetActor
    L7_2 = "15008"
    L5_2 = L5_2(L6_2, L7_2)
    L7_2 = L5_2
    L6_2 = L5_2.CallDelayByQuest
    L8_2 = "1500806"
    L9_2 = 5
    function L10_2(A0_3)
      local L1_3, L2_3, L3_3, L4_3
      L1_3 = actorMgr
      L2_3 = L1_3
      L1_3 = L1_3.GetActor
      L3_3 = "15008"
      L1_3 = L1_3(L2_3, L3_3)
      L3_3 = L1_3
      L2_3 = L1_3.EnablePlayerInputByQuest
      L4_3 = true
      L2_3(L3_3, L4_3)
    end
    L6_2(L7_2, L8_2, L9_2, L10_2)
  elseif L2_2 == 1 then
    L3_2 = actorMgr
    L4_2 = L3_2
    L3_2 = L3_2.GetActor
    L5_2 = "15008"
    L3_2 = L3_2(L4_2, L5_2)
    L5_2 = L3_2
    L4_2 = L3_2.TransmitPlayerByQuestId
    L6_2 = 1500806
    L7_2 = 7
    L8_2 = nil
    function L9_2(A0_3)
      local L1_3, L2_3, L3_3, L4_3, L5_3
      L1_3 = actorMgr
      L2_3 = L1_3
      L1_3 = L1_3.GetActor
      L3_3 = "15008"
      L1_3 = L1_3(L2_3, L3_3)
      L3_3 = L1_3
      L2_3 = L1_3.SetQuestVarByMainId
      L4_3 = 4
      L5_3 = 0
      L2_3(L3_3, L4_3, L5_3)
    end
    L10_2 = nil
    L4_2(L5_2, L6_2, L7_2, L8_2, L9_2, L10_2)
  end
end
L1_1.TriggerIn = L8_1
function L8_1(A0_2)
  local L1_2, L2_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - PreTriggerIn"
  L1_2(L2_2)
end
L1_1.PreTriggerIn = L8_1
function L8_1(A0_2)
  local L1_2, L2_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - LightUpTriggerIn"
  L1_2(L2_2)
end
L1_1.LightUpTriggerIn = L8_1
function L8_1(A0_2)
  local L1_2, L2_2
  L1_2 = print
  L2_2 = "Q1500806outtrigger - TriggerOut"
  L1_2(L2_2)
end
L1_1.TriggerOut = L8_1
return L1_1
